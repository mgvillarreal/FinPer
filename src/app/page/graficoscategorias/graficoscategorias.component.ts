...
export class GraficoscategoriasComponent implements OnInit {
  ...
  traeDatosPrincipal(mesActualPrincipal, anioActualPrincipal)
  {
    this.movimientoService.traeMovimientosGraficoCategorias(localStorage.getItem('id'), mesActualPrincipal+1, anioActualPrincipal)
    .pipe(
      catchError((error: any) => {
        //this.muestraGraficoFlag = 0;
        return throwError(error);
